...
이 문서는 **2026-09-06 코드 기준**입니다. 기존 제품 디자인을 유지하며, `design-review/`의 A/B/C HTML은 참고용 목업입니다.
...
원시 패킷부터 실제 게임 코드까지의 통합 검사와 측정 CSV 재생을 실행했습니다. 2026-09-06에는 센서를 연결하지 않은 ESP32-D0WD-V3에 펌웨어를 업로드하고, USB 50Hz 출력과 실제 BLE 약 20Hz 수신·재연결을 확인했습니다. Chrome 화면의 보정·게임 검사는 GATT 연결 부분에 테스트 신호를 주입한 결과입니다. 실제 압력 센서로 하는 Chrome/Edge 전체 흐름과 PostgreSQL 실행은 아직 별도 확인이 필요합니다. 서버는 제출 결과로 별점·XP를 재계산하지만, 센서 입력의 진위 자체를 인증하지는 않습니다. 별도 EMG 연구 결과도 현재 게임의 센서 성능을 입증하지 않습니다.
